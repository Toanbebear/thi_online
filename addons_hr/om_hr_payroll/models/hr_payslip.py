...
class HrPayslip(models.Model):
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            work_day = self.env['hr.attendance'].read_group([('check_in', '>=', day_from),
                                                         ('check_out', '<=', day_to),
                                                         ('employee_id', '=', contract.employee_id.id),
                                                         ('state', '=', 'validate')], ['workday'], ['workday'])
            number_of_days = 0
            for item in work_day:
                if item['workday'] == '1':
                    number_of_days += item['workday_count']
                elif item['workday'] == '1.5':
                    number_of_days += item['workday_count'] * 1.5
                else:
                    number_of_days += item['workday_count'] * 0.5
            ...
            w_d = {
                ...
                'number_of_hours': number_of_days * contract.resource_calendar_id.hours_per_day,
                'contract_id': contract.id,
            }
...
class HrPayslipLine(models.Model):
    ...
    period_name = fields.Char(string='Kỳ lương', compute='_compute_slip_id')

    def _compute_slip_id(self):
        for item in self:
            ttyme = datetime.combine(fields.Date.from_string(item.slip_id.date_from), time.min)
            locale = self.env.context.get('lang') or 'en_US'
            item.period_name = ('Kỳ lương tháng %s') % (tools.ustr(babel.dates.format_date(date=ttyme, format='MMMM-y', locale=locale)))
