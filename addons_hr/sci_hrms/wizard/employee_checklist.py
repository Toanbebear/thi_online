...
class RecruitmentPoint(models.TransientModel):
    _name = "sci.hr.employee.checklist"
    ...
    def open_report(self):
        ...
        tree_view_id = self.env.ref('sci_hrms.view_employee_entry_checklist_tree').id
        form_view_id = self.env.ref('hr.view_employee_form').id
        return {
            'type': 'ir.actions.act_window',
            'views': [(tree_view_id, 'tree'), (form_view_id, 'form')],
            'view_mode': 'tree,form',
            'name': 'Danh sách hồ sơ đầu vào',
            ...
            'context': dict(self.env.context, start_date=start_date, end_date=end_date,
                            search_default_group_hr_user_id=1, search_default_entry_progress=1),
        }
