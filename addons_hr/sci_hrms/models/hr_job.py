...
class JobPosition(models.Model):
    ...
    _sql_constraints = [
        ('name_uniq', 'unique (name)', "Tên chức vụ đã tồn tại !!!"),
    ]
...
class HRJob(models.Model):
    ...
    #     if self.total_payroll == self.no_of_employee:
    #         self.state = 'open'
    ...
    def _get_state(self):
        ...
        for record in self:
            ...
            elif record.periods and record.periods[-1].end_date and (record.periods[-1].end_date < datetime.now().date()):
                record.state = 'open'
                record.website_published = False
                record.is_published = False
            else:
