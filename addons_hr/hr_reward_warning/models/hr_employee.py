...
class HrAnnouncements(models.Model):
    ...
    def _announcement_count(self):
        for obj in self:
            announcement_ids = self.env['hr.announcement'].sudo().search(['|', ('employee_ids', 'in', [obj.id]), ('is_announcement', '=', True),
                                                                          ('state', 'in', ('approved', 'done'))])
            obj.announcement_count = len(announcement_ids)

    def announcement_view(self):
        for obj in self:
            ann_obj = self.env['hr.announcement'].sudo().search(['|', ('employee_ids', 'in', [obj.id]), ('is_announcement', '=', True),
                                                                 ('state', 'in', ('approved', 'done'))])
            ann_ids = []
            for each in ann_obj:
