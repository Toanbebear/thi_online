...
class EmployeeInsurance(models.Model):
    ...
    policy_id = fields.Many2one('insurance.policy', string='Chính sách', required=True, help="Policy")
    amount = fields.Float(string='Trích bảo hiểm', required=True, help="Policy amount")
    sum_insured = fields.Float(string="Lương tham gia BH", required=True, help="Insured sum")
    policy_coverage = fields.Selection([('monthly', 'Tháng'), ('yearly', 'Năm')],
                                       required=True, default='monthly',
                                       string='Phạm vi', help="During of the policy")
    ...
    state = fields.Selection([('active', 'Active'),
                              ('expired', 'Expired'), ],
                             default='active', string="Trạng thái",compute='get_status')
...
class HrInsurance(models.Model):
    ...
    insurance_percentage = fields.Float(string="Tỷ lệ đóng", help="Company insurance percentage")
    deduced_amount_per_month = fields.Float(string="Lương trừ mỗi tháng", compute="get_deduced_amount", help="Amount that is deduced from the salary per month")
    deduced_amount_per_year = fields.Float(string="Lương khấu trừ năm ", compute="get_deduced_amount", help="Amount that is deduced fronm the salary per year")
    insurance = fields.One2many('hr.insurance', 'employee_id', string="Bảo hiểm", help="Insurance",
                                domain=[('state', '=', 'active')])
