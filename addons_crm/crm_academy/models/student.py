...
class OpStudent(models.Model):
    ...
    count_sale_orders = fields.Integer('Invoice', compute='_count_sale_order')

    @api.depends('partner_id')
    def _count_sale_order(self):
        for record in self:
            record.count_sale_orders = len(self.env['sale.order'].search(
                [('partner_id', '=', record.partner_id.id), ('brand_id.type', '=', 'academy'),
