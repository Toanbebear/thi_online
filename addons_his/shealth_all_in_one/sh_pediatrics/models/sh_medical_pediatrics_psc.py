...
class SHealthPediatricSymptomsChecklist(models.Model):
    _name = "sh.medical.pediatrics.psc"
    ...
    doctor = fields.Many2one('sh.medical.physician', string='Physician', domain=[('is_pharmacist','=',False)], help="Current primary care / family doctor", required=True)
    ...
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.pediatrics.psc')
        vals['name'] = sequence
        return super(SHealthPediatricSymptomsChecklist, self).create(vals)
...
class SHealthPatient(models.Model):
    ...
    pediatrics_psc_ids = fields.One2many('sh.medical.pediatrics.psc', 'patient', string='Pediatrics Symptom Checklist')
