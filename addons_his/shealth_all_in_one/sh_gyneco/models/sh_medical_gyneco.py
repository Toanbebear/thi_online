...
class SHealthPerinatalMonitor(models.Model):
    _name = "sh.medical.perinatal.monitor"
    ...
    gyneco_id = fields.Many2one('sh.medical.gyneco', string='Gynecology', ondelete='cascade')

    @api.model
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.perinatal.monitor')
        vals['name'] = sequence or '/'
        return super(SHealthPerinatalMonitor, self).create(vals)


# Puerperium Monitor Management
class SHealthPuerperiumMonitor(models.Model):
    _name = "sh.medical.puerperium.monitor"
    ...
    @api.model
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.puerperium.monitor')
        vals['name'] = sequence or '/'
        return super(SHealthPuerperiumMonitor, self).create(vals)


# Gynecology Management
class SHealthGyneco(models.Model):
    _name = "sh.medical.gyneco"
    ...
    perinatal_ids = fields.One2many('sh.medical.perinatal.monitor','gyneco_id', string='Perinatal')
    puerperium_ids = fields.One2many('sh.medical.puerperium.monitor','gyneco_id', string='Puerperium')
    ...
    patient = fields.Many2one('sh.medical.patient', string='Patient', ondelete='cascade')


    @api.model
    def create(self, vals):
        sequence = self.env['ir.sequence'].next_by_code('sh.medical.gyneco')
        vals['name'] = sequence or '/'
        return super(SHealthGyneco, self).create(vals)
...
class SHealthPatient(models.Model):
    ...
    gyneco_ids = fields.One2many('sh.medical.gyneco', 'patient', string='Perinatal')
