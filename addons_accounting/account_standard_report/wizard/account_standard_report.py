...
class AccountInvoiceLine(models.Model):
    ...
    move_line_rec_id = fields.Many2one('account.move.line', string="Move line đối ứng", compute="compute_move_id", store=True)
...
class AccountStandardLedgerLines(models.TransientModel):
    ...
    account_rec_id = fields.Many2one('account.account', 'Đối ứng')
    ...
    move_line_rec_id = fields.Many2one('account.move.line',string="Move line đối ứng")
    date = fields.Date()
    ...
    # def compute_move_line_id(self):
    #     for line in self:
    #         reconcile_line = line.move_line_id.move_id.line_ids.search([('move_id','=',line.move_line_id.move_id.id),('account_id','not in',[line.move_line_id.account_id.id])],limit=1)
    #         line.move_line_rec_id = reconcile_line
    #         line.account_rec_id = reconcile_line.account_id
...
class AccountStandardLedger(models.TransientModel):
    ...
    account_in_ex_clude_ids = fields.Many2many(comodel_name='account.account', string='Accounts', domain="[('company_id','=',company_id)]",
    ...
    journal_ids = fields.Many2many('account.journal', string='Journals', required=True, domain="[('company_id','=',company_id)]",
    ...
    @api.onchange('ledger_type', 'result_selection')
    ...
    def _sql_lines(self):
        ...
        query = """
        INSERT INTO account_report_standard_ledger_line
            (report_id, create_uid, create_date, account_id, account_rec_id, analytic_account_id, line_type, view_type, journal_id, partner_id, move_id, move_line_id, move_line_rec_id, date, date_maturity, debit, credit, balance, full_reconcile_id, reconciled, report_object_id, cumul_balance, current, age_30_days, age_60_days, age_90_days, age_120_days, older, company_currency_id, amount_currency, currency_id)
    ...
    def _sql_get_line_for_report(self, type_l, report_object=None):
        ...
        query = """SELECT
                    ...
                    acc.code AS a_code,
                    acc.name AS a_name,
                    acc_rec.code AS a_r_code,
                    acc_rec.name AS a_r_name,
                    ...
                    CASE 
                        WHEN rep.code_customer IS NOT NULL THEN CONCAT('[',rep.code_customer,']',rep.display_name)
                        ELSE rep.display_name
                    END AS partner_name,
                    raml.date AS date,
                ...
                FROM
                    ...
                    LEFT JOIN account_account acc_rec ON (acc_rec.id = raml.account_rec_id)
