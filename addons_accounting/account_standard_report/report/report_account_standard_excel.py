        ...
        if wizard.ledger_type == 'aged':
            ...
            if wizard.summary:
                ...
                def _set_line(line):
                    ...
                    sheet.write(i, 4, line.get('a_r_code', ''))
                    sheet.write(i, 5, line.get('a_r_name', ''))
                    ...
                    sheet.write(i, 9, _get_data_float(line.get('current')), currency_format)
                    sheet.write(i, 10, _get_data_float(line.get('age_30_days')), currency_format)
                    sheet.write(i, 11, _get_data_float(line.get('age_60_days')), currency_format)
                    sheet.write(i, 12, _get_data_float(line.get('age_90_days')), currency_format)
                    sheet.write(i, 13, _get_data_float(line.get('age_120_days')), currency_format)
                    sheet.write(i, 14, _get_data_float(line.get('older')), currency_format)
                    sheet.write(i, 15, _get_data_float(line.get('balance')), currency_format)
            ...
            if wizard.summary:
                ...
                def _set_line(line):
                    sheet.write(i, 0, get_date_format(line.get('date', '')) if line.get('view_type') != 'init' else 'ĐẦU KỲ')
                    sheet.write(i, 1, line.get('j_code', ''))
                    sheet.write(i, 2, line.get('a_code', ''))
                    sheet.write(i, 3, line.get('a_name', ''))
                    sheet.write(i, 4, line.get('a_r_code', ''))
                    sheet.write(i, 5, line.get('a_r_name', ''))
                    sheet.write(i, 6, "%s - %s" % (line.get('an_code', ''), line.get('an_name', '')) if line.get('an_code', '') else line.get('an_name', ''))
                    sheet.write(i, 7, line.get('move_name', ''))
                    sheet.write(i, 8, line.get('displayed_ref', ''))
                    sheet.write(i, 9, line.get('displayed_name', ''))
                    sheet.write(i, 10, line.get('partner_name', ''))
                    sheet.write(i, 11, get_date_format(line.get('date_maturity', '')))
                    sheet.write(i, 12, _get_data_float(line.get('debit', '')), currency_format)
                    sheet.write(i, 13, _get_data_float(line.get('credit', '')), currency_format)
                    sheet.write(i, 14, _get_data_float(line.get('cumul_balance', '')), currency_format)
                ...
                for obj in report.report_object_ids:
                    ...
                    for line in all_lines:
                        if line.get('report_object_id') == obj_id:
                            if line.get('partner_name') == partner_name and line.get('view_type') == 'init':
                                line['j_code'] = ''
                                line['a_r_code'] = ''
                                line['a_r_name'] = ''
                                line['move_name'] = ''
                                ...
                                if line['cumul_balance'] > 0:
                                    line['debit'] = line['cumul_balance']
                                    line['credit'] = 0
                                else:
                                    line['credit'] = -line['cumul_balance']
                                    line['debit'] = 0
                    ...
                    if lines_obj:
                        ...
                        if wizard.report_type == 'partner':
                            name_view = '[%s]%s' % (obj.partner_id.code_customer,obj.partner_id.display_name)
