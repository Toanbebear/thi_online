...
class OpAttendanceLine(models.Model):
    ...
    catch_up = fields.Many2one('op.session', 'Catch up')
    session_content = fields.Many2one('op.session.content', string='Content',
                                      related='attendance_id.session_id.session_content', store=True)
    student_detail = fields.Many2one('op.student.course', string='Student details')
