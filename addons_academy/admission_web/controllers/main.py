...
class AdmissionWeb(http.Controller):
    # --------------------------------------------------
    # MAIN / SEARCH
    # --------------------------------------------------
    ...
    @http.route([
        '/ofcourses',
        '/ofcourses/page/<int:page>',
        '/ofcourses/category/<model("op.category"):category>',
        '/ofcourses/category/<model(\
        "op.category"):category>/page/<int:page>'
    ...
    def courses(self, search='', category=False, page=0, ppg=False, **post):
        ...
        if category:
            url = "/ofcourses/category/%s" % slug(category)
            domain += [('category_ids', 'in', [category.id])]
        course_domain = domain + [('state','=','application')]
        course_ref = request.env['op.admission.register']
        ...
        course_category_ref = request.env['op.category']
        if category:
        ...
        return request.render("admission_web.ofcourses", values)
    ...
    @http.route('''/ofcourse-detail/<model("op.admission.register"):course>''', type='http',
                auth="public", website=True)
    def course(self, course, **kw):
        course = request.env['op.admission.register'].sudo().browse([course.id])
        ...
        return request.render('admission_web.ofcourse_detail', values)
