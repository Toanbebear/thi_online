...
class OpMarksheetLine(models.Model):
    ...
    name = fields.Char('Marksheet name', compute='_get_name')
    ...
    batch_id = fields.Many2one('op.batch', 'Batch', compute='_get_batch', store=True)

    @api.depends('marksheet_reg_id')
    def _get_batch(self):
        for record in self:
            record.batch_id = record.marksheet_reg_id.exam_session_id.batch_id

    @api.depends('student_id', 'batch_id')
    def _get_name(self):
        for record in self:
            record.name = str(self.student_id.name) + " / " + str(record.batch_id.name)
