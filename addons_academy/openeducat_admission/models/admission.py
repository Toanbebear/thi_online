...
class OpAdmission(models.Model):
    ...
    birth_date = fields.Date(
        'Birth Date', states={'done': [('readonly', True)]})
    ...
    bom = fields.Many2one('product.bundle', string='BOM')
    ...
    def get_student_vals(self):
        for student in self:
            ...
            details.update({
                ...
                'name': student.name,
                'student_id': self.env['ir.sequence'].next_by_code('op.student'),
    ...
    def enroll_student(self):
        ...
        # link bom and inventory
            if record.bom:
                vals = []
                institute_location = record.institute.location
                warehouse = institute_location.get_warehouse()
                picking_type = warehouse.out_type_id
                for line in record.bom.products:
                    qty_on_hand = self.env['stock.quant']._get_available_quantity(product_id=line.product, location_id=institute_location)
                    ...
                    if qty_on_hand < line_qty:
                        raise ValidationError(_("%s out of stock in %s.\
                                                 Please contact inventory manager." % (line.product.name, institute_location)))
                    else:
                        vals.append((0, 0, {'name': 'BOM' + line.product.name,
                                            ...
                                            'location_dest_id': record.student_id.partner_id.property_stock_customer.id}))
                pick_name = 'Student BOM - %s - %s' % (record.name, record.course_id.name)
