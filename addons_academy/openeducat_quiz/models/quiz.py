...
class OpQuiz(models.Model):
    ...
    def update_student(self):
        list_student = []
        if self.batch_ids:
            for rec in self.batch_ids.student_course.student_id:
                list_student.append(rec)
                self.student_id += rec

    def action_init(self):
        return {
            'name': 'Cấu hình danh sách thi',
            'type': 'ir.actions.act_window',
            'view_type': 'form',
            'view_mode': 'form',
            'view_id': self.env.ref('openeducat_quiz.view_employee_wizard_form').id,
            'res_model': 'employee.wizard',
            'context': {
                'default_employee_id': self.employee_id.ids,
                'default_company_id': self.company_id.ids,
                'default_quiz_id': self.id,
            },
            'target': 'new',
        }

    @api.depends('exam_id', 'exam_id.attendees_line')
    def _compute_quiz_users(self):
        for record in self:
            if record.exam_id:
                user_list = []
                for val in record.exam_id.attendees_line:
                    if val.user_id:
                        user_list.append(val.user_id.id)
                record.user_ids = user_list
            else:
    ...
    quiz_employee = fields.Boolean('Employee exam')
    academy_exam = fields.Boolean('Academy exam')
    student_id = fields.Many2many('op.student', string='Student', domain="[('company_id', '=', company_id)]")
