...
class OpQuizResult(models.Model):
    ...
    def get_answer_data(self):
        ...
        return {
            ...
            'user': self.env.user,
            'bai_thi': self.name,
            'thoi_gian': self.finish_date + timedelta(hours=7),
        }
        ...
        for obj in self:
            ...
            for line in obj.line_ids:
                ...
                if line.que_type == 'optional':
                    ...
                    if line.mark == 0:
                        total_incorrect += 1
            self.env['op.result.line'].search(
                [('exam_id', '=', obj.quiz_id.exam_id.id),
                 ('user_id', '=', obj.user_id.id)]).sudo().write({'online_marks': received_marks or 0})
    ...
    state = fields.Selection([
        ...
        ('cancel', 'Cancel')], string='Trạng thái')
    ...
    create_on = fields.Datetime('Ngày tạo', default=lambda self: fields.Datetime.now())

    classify_result = fields.Selection([
        ('achieve', 'Đạt'), ('not_achieved', 'Không đạt')
    ], string='Kết quả', compute="_get_classify_result", store=True)

    @api.depends('score')
    def _get_classify_result(self):
        for rec in self:
            if rec.score >= 80:
                rec.write({
                    'classify_result': 'achieve',
                })
            else:
                rec.write({
                    'classify_result': 'not_achieved',
                })
    # @api.depends('line_ids', 'line_ids.mark', 'line_ids.answer',
    #              'line_ids.given_answer')
    # def _get_result(self):
