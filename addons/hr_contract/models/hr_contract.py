...
class Contract(models.Model):
    ...
    employee_id = fields.Many2one('hr.employee', string='Employee', tracking=True, domain="[('company_id', '=', company_id)]")
    ...
    def _check_current_contract(self):
        ...
        for contract in self.filtered(lambda c: c.state not in ['draft', 'cancel'] or c.state == 'draft' and c.kanban_state == 'done'):
            ...
            if self.search_count(domain):
                raise ValidationError('Một nhân viên chỉ có thể có một hợp đồng cùng một lúc. (Không bao gồm HĐ nháp và HĐ bị hủy')
