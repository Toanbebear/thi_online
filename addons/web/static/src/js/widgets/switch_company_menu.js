...
var SwitchCompanyMenu = Widget.extend({
    ...
    willStart: function () {
        ...
        this.all_companies = this.allowed_company_ids.length == this.user_companies.length;
    ...
    _onToggleAllCompanyClick: function (ev) {
        ...
        var allowed_company_ids = this.allowed_company_ids;
        var current_company_id = allowed_company_ids[0];
        // Chọn tất cả các công ty
        if (dropdownItem.find('.fa-square-o').length) {
            dropdownItem.find('.fa-square-o').removeClass('fa-square-o').addClass('fa-check-square');
            var company_ids = [];
            for (var index in this.user_companies){
                company_ids.push(this.user_companies[index][0])
            }
            allowed_company_ids = company_ids;
            session.setMultiCompanies(company_ids);
        } else {
            // Chọn công ty đầu tiên
            dropdownItem.find('.fa-check-square').addClass('fa-square-o').removeClass('fa-check-square');
            allowed_company_ids = [current_company_id];
            session.setCompanies(current_company_id, [current_company_id]);
        }
    },
